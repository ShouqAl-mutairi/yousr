...
if (window.location.pathname.includes('/contact')) {
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', async (e) => {
            ...
            if (firstName.length < 3) errors.push('يرجى كتابة الاسم الأول بشكل صحيح.');
            if (lastName.length < 3) errors.push('يرجى كتابة الاسم الأخير بشكل صحيح.');
            if (!/^[0-9]{10,15}$/.test(phone)) errors.push('يرجى إدخال رقم هاتف صحيح.');
            if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) errors.push('يرجى إدخال بريد إلكتروني صحيح.');
            if (!dob) errors.push('يرجى إدخال تاريخ الميلاد.');
            if (!language) errors.push('يرجى اختيار اللغة.');
            if (!message || message.length < 10) errors.push('يرجى كتابة رسالة لا تقل عن 10 أحرف.');
            if (!gender) errors.push('يرجى اختيار الجنس.');

            if (errors.length > 0) {
                showNotification('error', 'خطأ في النموذج', errors.join('<br>'));
                return;
            }
            ...
            try {
                ...
                if (response.ok) {
                    showNotification('success', 'تم الإرسال بنجاح', 'شكراً لتواصلك معنا!');
                    contactForm.reset();
                    setTimeout(() => window.location.href = '/', 2000);
                } else {
                    showNotification('error', 'فشل الإرسال', result.message || 'حدث خطأ غير متوقع.');
                    submitButton.disabled = false;
